...
async function askCsAgentEnablement(projectRoot) {
  let detected;
  let probed;
  try {
    probed = await probeContentServer({ project: projectRoot });
    detected = await detectContentServer({
      project: projectRoot,
      executable: probed.data.executable,
    });
  } catch {
    return {
      available: false,
  ...
  data.executableTrust ??= probed?.data?.executableTrust;
...
function createCsAgentFingerprint(data, profile) {
  return {
