  ...
  applyCsAgentAnswers(answers) {
    ...
    if (existsSync(configPath)) {
      upsertTomlSection(configPath, 'integrations.cs_agent', {
        enabled: csAgent.enabled === true,
        profile: csAgent.enabled === true ? csAgent.profile : '',
        executable: csAgent.enabled === true ? csAgent.executable : '',
        context_dir: '.reversa/context/cs-agent',
        inventory_path: csAgent.enabled === true ? `${answers.output_folder}/inventory.md` : '',
        snapshot_ttl_days: 7,
    ...
    if (existsSync(statePath)) {
      ...
      state.cs_agent_enablement_dismissed = csAgent.dismissed ?? null;
      writeFileSync(statePath, JSON.stringify(state, null, 2), 'utf8');
    }

    if (csAgent.enabled === true) {
