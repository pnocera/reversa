import test from 'node:test';
import assert from 'node:assert/strict';
import {
  mkdirSync,
  mkdtempSync,
  readFileSync,
  rmSync,
  writeFileSync,
} from 'fs';
import { tmpdir } from 'os';
import { join } from 'path';
import { fileURLToPath } from 'url';
import { dirname } from 'path';
import { __test } from '../../integrations/cs-agent.js';
import contentServer, {
  detect,
  inventory,
  snapshot,
} from '../content-server.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
const FIXTURES = join(__dirname, '..', '..', 'integrations', '__tests__', 'fixtures');

function fixture(name) {
  return readFileSync(join(FIXTURES, name), 'utf8');
}

function tempProject() {
  return mkdtempSync(join(tmpdir(), 'reversa-content-server-test-'));
}

function fakeExecutable(projectRoot) {
  const exe = join(projectRoot, process.platform === 'win32' ? 'cs-agent.exe' : 'cs-agent');
  writeFileSync(exe, 'fake', 'utf8');
  return exe;
}

function okResult(stdout) {
  return {
    stdout,
    stderr: '',
    exitCode: 0,
    timedOut: false,
    durationMs: 1,
  };
}

function stubRunner(calls) {
  return async (_executable, args) => {
    calls.push(args);
    const command = args.slice(0, 2).join(' ');
    if (args[0] === 'help') return okResult(fixture('help.json'));
    if (command === 'profile info') return okResult(fixture('profile-info.json'));
    if (command === 'graph status') return okResult(fixture('graph-status.json'));
    if (command === 'docs categories') return okResult(fixture('docs-categories.json'));
    return okResult(JSON.stringify({ ok: false, error: { message: 'unexpected' } }));
  };
}

test.afterEach(() => {
  __test.resetRunCmd();
  process.exitCode = undefined;
});

test('detect returns profiles and executable trust', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const calls = [];
  __test.setRunCmd(stubRunner(calls));

  const envelope = await detect({
    project: projectRoot,
    executable: exe,
  });

  assert.equal(envelope.ok, true);
  assert.equal(envelope.action, 'detect');
  assert.equal(envelope.data.active, 'CS253');
  assert.equal(envelope.data.executableTrust.path, exe);
  assert.deepEqual(calls.map(args => args.slice(0, 2).join(' ')), ['help --json', 'profile info']);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('snapshot can run in explicit test mode without enabled config', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const outDir = join(projectRoot, 'snapshot');
  __test.setRunCmd(stubRunner([]));

  const envelope = await snapshot({
    project: projectRoot,
    executable: exe,
    profile: 'CS253',
    out_dir: outDir,
  });

  assert.equal(envelope.ok, true);
  assert.equal(envelope.action, 'snapshot');
  assert.equal(envelope.data.snapshotDir, outDir);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('CLI snapshot accepts --ttl-days and quiet suppresses success output', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const outDir = join(projectRoot, 'snapshot');
  __test.setRunCmd(stubRunner([]));

  const captured = await captureConsole(() => contentServer([
    'snapshot',
    '--project', projectRoot,
    '--executable', exe,
    '--profile', 'CS253',
    '--out-dir', outDir,
    '--ttl-days', '3',
    '--quiet',
  ]));

  assert.equal(process.exitCode, 0);
  assert.equal(captured.stdout, '');
  assert.equal(captured.stderr, '');
  const meta = JSON.parse(readFileSync(join(outDir, '_meta.json'), 'utf8'));
  assert.equal(meta.snapshot_ttl_days, 3);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('inventory writes a marked block from an explicit snapshot directory', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const outDir = join(projectRoot, 'snapshot');
  const inventoryPath = join(projectRoot, 'inventory.md');
  __test.setRunCmd(stubRunner([]));

  await snapshot({
    project: projectRoot,
    executable: exe,
    profile: 'CS253',
    out_dir: outDir,
  });

  const envelope = await inventory({
    project: projectRoot,
    snapshot_dir: outDir,
    inventory_path: inventoryPath,
  });

  assert.equal(envelope.ok, true);
  assert.match(readFileSync(inventoryPath, 'utf8'), /CS Profile \(cs-agent\)/);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('snapshot requires enabled config unless explicit flags are present', async () => {
  const projectRoot = tempProject();
  await assert.rejects(
    () => snapshot({ project: projectRoot, profile: 'CS253' }),
    err => err.code === 'cs_agent_not_enabled',
  );
  rmSync(projectRoot, { recursive: true, force: true });
});

test('snapshot reads enabled config and config.user.toml executable override', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const calls = [];
  __test.setRunCmd(async (executable, args) => {
    calls.push({ executable, args });
    const command = args.slice(0, 2).join(' ');
    if (args[0] === 'help') return okResult(fixture('help.json'));
    if (command === 'profile info') return okResult(fixture('profile-info.json'));
    if (command === 'graph status') return okResult(fixture('graph-status.json'));
    if (command === 'docs categories') return okResult(fixture('docs-categories.json'));
    return okResult(JSON.stringify({ ok: false, error: { message: 'unexpected' } }));
  });

  mkdirSync(join(projectRoot, '.reversa'), { recursive: true });
  writeFileSync(join(projectRoot, '.gitignore'), '', 'utf8');
  writeFileSync(join(projectRoot, '.reversa', 'config.toml'), [
    '[output]',
    'folder = "_reversa_sdd"',
    '',
    '[integrations.cs_agent]',
    'enabled = true',
    'profile = "CS253"',
    'executable = ""',
    'context_dir = ".reversa/context/cs-agent"',
    'inventory_path = "_reversa_sdd/inventory.md"',
    'snapshot_ttl_days = 7',
    '',
  ].join('\n'), 'utf8');
  writeFileSync(join(projectRoot, '.reversa', 'config.user.toml'), [
    '[integrations.cs_agent]',
    `executable = "${exe.replace(/\\/g, '\\\\')}"`,
    '',
  ].join('\n'), 'utf8');

  const envelope = await snapshot({ project: projectRoot });

  assert.equal(envelope.ok, true);
  assert.equal(calls[0].executable, exe);
  assert.deepEqual(calls[0].args, ['help', '--json']);
  assert.ok(calls.slice(1).every(call => call.args.includes('--profile') && call.args.includes('CS253')));
  assert.match(readFileSync(join(projectRoot, '.gitignore'), 'utf8'), /^\.reversa\/context\/cs-agent\/\s*$/m);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('subcommand help documents action-specific flags', async () => {
  const captured = await captureConsole(() => contentServer(['snapshot', '--help']));

  assert.match(captured.stdout, /Usage: reversa content-server snapshot/);
  assert.match(captured.stdout, /--ttl-days <int>/);
  assert.match(captured.stdout, /--allow-disabled/);
});

test('CLI maps snapshot partial failures to exit code 4', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  const outDir = join(projectRoot, 'snapshot');
  __test.setRunCmd(async (_executable, args) => {
    const command = args.slice(0, 2).join(' ');
    if (args[0] === 'help') return okResult(fixture('help.json'));
    if (command === 'profile info') return okResult(fixture('profile-info.json'));
    if (command === 'graph status') {
      return {
        stdout: '',
        stderr: 'graph failed',
        exitCode: 1,
        timedOut: false,
        durationMs: 1,
      };
    }
    return okResult(fixture('docs-categories.json'));
  });

  await captureConsole(() => contentServer([
    'snapshot',
    '--project', projectRoot,
    '--executable', exe,
    '--profile', 'CS253',
    '--out-dir', outDir,
    '--json',
  ]));

  assert.equal(process.exitCode, 4);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('CLI maps inventory render blocking to exit code 5', async () => {
  const projectRoot = tempProject();
  await captureConsole(() => contentServer([
    'inventory',
    '--project', projectRoot,
    '--snapshot-dir', join(projectRoot, 'missing-snapshot'),
    '--inventory-path', join(projectRoot, 'inventory.md'),
    '--json',
  ]));

  assert.equal(process.exitCode, 5);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('CLI maps generic adapter failures and unknown actions to exit code 3', async () => {
  const projectRoot = tempProject();
  const exe = fakeExecutable(projectRoot);
  __test.setRunCmd(async () => ({
    stdout: '',
    stderr: 'boom',
    exitCode: 1,
    timedOut: false,
    durationMs: 1,
  }));

  await captureConsole(() => contentServer([
    'probe',
    '--project', projectRoot,
    '--executable', exe,
    '--json',
  ]));
  assert.equal(process.exitCode, 3);

  process.exitCode = undefined;
  await captureConsole(() => contentServer(['unknown-action', '--json']));
  assert.equal(process.exitCode, 3);

  rmSync(projectRoot, { recursive: true, force: true });
});

test('CLI rejects broad allow-disabled and conflicting inventory modes', async () => {
  const projectRoot = tempProject();

  await captureConsole(() => contentServer([
    'snapshot',
    '--project', projectRoot,
    '--profile', 'CS253',
    '--allow-disabled',
    '--json',
  ]));
  assert.equal(process.exitCode, 3);

  process.exitCode = undefined;
  await captureConsole(() => contentServer([
    'inventory',
    '--project', projectRoot,
    '--write',
    '--print',
    '--json',
  ]));
  assert.equal(process.exitCode, 3);

  rmSync(projectRoot, { recursive: true, force: true });
});

async function captureConsole(fn) {
  const oldLog = console.log;
  const oldError = console.error;
  const stdout = [];
  const stderr = [];
  console.log = value => { stdout.push(String(value)); };
  console.error = value => { stderr.push(String(value)); };
  try {
    const result = await fn();
    return {
      result,
      stdout: stdout.join('\n'),
      stderr: stderr.join('\n'),
    };
  } finally {
    console.log = oldLog;
    console.error = oldError;
  }
}
