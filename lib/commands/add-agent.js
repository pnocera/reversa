...
export default async function addAgent(args) {
  ...
  if (!existing.installed) {
    ...
    console.log('  Run ' + chalk.bold('npx @pnocera/reversa install') + ' to install.\n');
    return;
  }
  ...
  if (!Array.isArray(state.engines) || state.engines.length === 0) {
    ...
    console.log('  Run ' + chalk.bold('npx @pnocera/reversa install') + ' or ' + chalk.bold('npx @pnocera/reversa add-engine') + ' first.\n');
    return;
  }
