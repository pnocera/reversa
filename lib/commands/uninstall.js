...
export default async function uninstall(args) {
  ...
  const csAgentContextDir = join(projectRoot, '.reversa', 'context', 'cs-agent');
  const hasCsAgentContext = existsSync(csAgentContextDir);
  ...
  console.log(chalk.bold('\n  Folders:'));
  if (hasCsAgentContext) {
    console.log(chalk.red('    ✗  .reversa/context/cs-agent/  (read-only cs-agent cache)'));
  }
