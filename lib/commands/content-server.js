...
export default async function contentServer(args) {
  ...
  if (!ACTIONS.has(parsed.action)) {
    const envelope = errorEnvelope(parsed.action, new AdapterError(
      'unknown_action',
      `Unknown content-server action: ${parsed.action}`,
    ));
    printEnvelope(envelope, parsed.flags.json);
    process.exitCode = 5;
    return;
  }
...
function ensureEnabledOrExplicit(context, flags, action) {
  ...
  if (flags.allow_disabled) return;
...
function printHelp() {
  console.log(`
  Uso: reversa content-server <acao> [opcoes]

  Acoes:
    probe       Verifica o executavel cs-agent e a arvore de ajuda JSON
    detect      Lista perfis Content Server sem escolher automaticamente
    snapshot    Coleta profile info, graph status e docs categories
    inventory   Renderiza ou atualiza o bloco de inventario do Content Server
    doctor      Recoleta snapshot e compara contagens basicas

  Opcoes:
    --json                       Emite um unico envelope JSON
    --executable <path>          Caminho do cs-agent/cs-agent.exe
    --profile <name>             Perfil cs-agent a usar
    --out-dir <path>             Diretorio do snapshot
    --snapshot-dir <path>        Diretorio do snapshot para inventory/doctor
    --inventory-path <path>      Markdown alvo do inventario
    --ttl <hours>                Janela de frescor do snapshot em horas
    --print                      Imprime o bloco de inventory sem escrever arquivo
    --allow-disabled             Permite snapshot/inventory/doctor sem enabled=true
    --project <path>             Raiz do projeto Reversa
  `);
}
