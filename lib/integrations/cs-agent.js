...
export function resolveExecutable(options = {}) {
  ...
  const configuredPath = normalized.configured || normalized.executable || normalized.executablePath || normalized.configuredPath;
  if (configuredPath) {
...
export async function runReadOnly(topicOrOptions = {}, commandArg, opts = {}) {
  const options = typeof topicOrOptions === 'string'
    ? { ...opts, topic: topicOrOptions, command: commandArg }
    : topicOrOptions;
  const {
    executable,
    executablePath,
  ...
  const executableToRun = executable || executablePath;

  if (!profile) {
    throw new AdapterError('profile_required', 'A cs-agent profile is required before running read-only commands');
  }
  if (!executableToRun) {
    throw new AdapterError('executable_not_found', 'A cs-agent executable path is required');
  }
  ...
  let audit = null;
  try {
    const result = await executeJson(executableToRun, argv, {
...
export async function collectSnapshot(options = {}) {
  ...
  try {
    ...
    successes.push('docs-categories');
    ...
    const snapshotTtlDays = Number.isInteger(options.snapshotTtlDays)
      ? options.snapshotTtlDays
      : Number.isInteger(options.ttlDays)
        ? options.ttlDays
        : 7;
    const meta = {
      ...
      snapshot_files: {
        profile_info: SNAPSHOT_FILES.profile,
        graph_status: SNAPSHOT_FILES.graph,
        docs_categories: SNAPSHOT_FILES.docs,
      },
      snapshot_ttl_days: snapshotTtlDays,
    };
    ...
    if (successes.length > 0) {
      throw new AdapterError('snapshot_partial', 'cs-agent snapshot failed before commit', {
        successes,
...
export function validateSnapshotMeta(snapshotDir, options = {}) {
  const { dir, metaPath } = resolveSnapshotTarget(snapshotDir);
  const problems = [];
  let meta = null;
  ...
  const files = snapshotFilesFromMeta(meta);
  ...
  const ttlHours = options.ttlHours
    ?? (Number.isInteger(options.snapshotTtlDays) ? options.snapshotTtlDays * 24 : null)
    ?? (Number.isInteger(meta.snapshot_ttl_days) ? meta.snapshot_ttl_days * 24 : null)
    ?? DEFAULT_TTL_HOURS;
  const createdAtRaw = meta.collected_at || meta.created_at;
...
export function renderInventorySection(options = {}) {
  ...
  if (!validation.ok) {
    throw new AdapterError('inventory_render_blocked', 'cs-agent snapshot is missing or invalid', {
      problems: validation.problems,
    });
  }

  const files = snapshotFilesFromMeta(validation.meta);
  ...
  const shouldWrite = mode === 'write' || (mode !== 'print' && inventoryPath);
  if (!shouldWrite) {
    return { block, validation, written: false, action: 'printed', warning: null };
  }
  ...
  writeTextAtomic(inventoryPath, next.text);
  return {
    block,
    validation,
    written: true,
...
function detectExecutableDrift(meta, options = {}) {
  ...
  const trustedSize = trusted.size_bytes ?? trusted.sizeBytes;
  const trustedMtimeMs = trusted.mtime_ms
    ?? (trusted.mtime_iso ? Date.parse(trusted.mtime_iso) : null)
    ?? (trusted.mtimeIso ? Date.parse(trusted.mtimeIso) : null);
  const trustedHelpSignature = trusted.help_signature_sha256 ?? trusted.helpSignatureSha256;
  const currentHelpSignature = options.currentHelpSignatureSha256;
  return Boolean(
    trustedSize && trustedSize !== current.size
    || trustedMtimeMs && Math.trunc(trustedMtimeMs) !== Math.trunc(current.mtimeMs)
    || currentHelpSignature && trustedHelpSignature && trustedHelpSignature !== currentHelpSignature
  );
}

function appendAuditLog(projectRoot, entry, options = {}) {
  try {
    const logPath = options.logPath || join(projectRoot, '.reversa', 'context', 'cs-agent', 'adapter.log');
    const dir = dirname(logPath);
    mkdirSync(dir, { recursive: true });
    const line = `${formatAuditEntry(entry)}\n`;
    if (existsSync(logPath) && statSync(logPath).size + Buffer.byteLength(line, 'utf8') > AUDIT_MAX_BYTES) {
      const lines = readFileSync(logPath, 'utf8').split(/\r?\n/).filter(Boolean);
      const keepFrom = Math.floor(lines.length * 0.25);
      writeFileSync(logPath, `${lines.slice(keepFrom).join('\n')}\n`, 'utf8');
    }
    writeFileSync(logPath, line, { encoding: 'utf8', flag: 'a' });
  } catch {
...
function upsertMarkedBlock(existing, block) {
  ...
  return { text: suffix, action: 'appended', warning: null };
}
...
function resolveSnapshotTarget(snapshotDirOrMetaPath) {
  const resolved = resolve(snapshotDirOrMetaPath);
  if (resolved.toLowerCase().endsWith('_meta.json')) {
    return { dir: dirname(resolved), metaPath: resolved };
  }
  return { dir: resolved, metaPath: join(resolved, '_meta.json') };
}

function snapshotFilesFromMeta(meta = {}) {
  const files = meta.files || {};
  const snapshotFiles = meta.snapshot_files || {};
  return {
    profile: files.profile || files.profile_info || snapshotFiles.profile || snapshotFiles.profile_info || SNAPSHOT_FILES.profile,
    graph: files.graph || files.graph_status || snapshotFiles.graph || snapshotFiles.graph_status || SNAPSHOT_FILES.graph,
    docs: files.docs || files.docs_categories || snapshotFiles.docs || snapshotFiles.docs_categories || SNAPSHOT_FILES.docs,
  };
}
