...
Independent agents (run at any phase): **Visor**, **Data Master**, **Design System**, **Soul Extractor**, **Content Server**, **Reconstructor**.
...
| **Content Server** | Reads initialized `cs-agent` snapshots for OpenText Content Server projects without walking huge source trees |
...
│   └── cs-agent/       # Optional read-only Content Server snapshot cache
└── _config/
