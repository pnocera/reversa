# Generative Patterns for the Seal

Catalog of the 5 established patterns that the `selo-generativo` skill produces. Each pattern has a distinct appearance, a central algorithm, and parameters derived from the seed.

General seed convention: the sha256 hash (64 hex chars) is cut into 8-char slices; each slice becomes an `parseInt(slice, 16)` and feeds a distinct parameter. This way, different patterns from the same seed share a visual personality.

---

## 1. flow-field

Perlin flow fields: thousands of particles follow vectors derived from noise, leaving organic curved trails. Style: "naturally turbulent".
...
- Gravity center of the field (if an attractor exists): XY coordinate (slices 2 and 3).
...
**When it fits**: `premium` style (dark, gold) and `exploratory` (luminous pastels).
...
Symmetric crystalline form derived from a base polygon, with clean geometric subdivisions. Style: "architectural logotype".
...
**SVG-exportable**: this pattern is purely geometric, ideal for conversion to real SVG for mini-seals.
...
Moire-type interference patterns: circular waves from multiple centers that cross, generating complex textures from simple rules.
...
**When it fits**: `sober` style (neutral terracotta) and `exploratory` (auroral).
...
- Base height of the horizon: 25% to 40% of the canvas (slice 1).
...
function pickPattern(seedHex, styleHint) {
    ...
    // Soft style adjustment (chooses among "compatible" patterns if there is a mismatch)
...
The `pattern x style` compatibility is shown at the beginning of this reference. When there is a declared incompatibility, the `pickCompatible` function re-evaluates among the patterns marked as appropriate for the style.

---

## Manual override

The skill accepts a `forcePattern` parameter to ignore the seed derivation and choose the pattern manually, useful when the user wants a specific seal in a different style from the default.
