...
1. **`paradigm_decision.md`**, non-negotiable first read. The target paradigm shapes how all coding must happen.
...
4. **Write tests** from `parity_specs.md` and `parity_tests/*.feature` from the start.
5. **For each component**, validate that it respects the chosen paradigm (explicit signals in `target_architecture.md § Honor to the chosen paradigm`).
